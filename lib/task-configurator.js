            ...
            .spread(function (nodes, targets) {
                ...
                if (args.length) {
                    args.forEach(function (arg) {
                        ...
                        nodes = filterTargets(arg, nodes);
                        targets = filterTargets(arg, targets);
                    });
...
TaskConfigurator.prototype._prebuild = function (args) {
    ...
    return vow.all(this._callbacks.map(function (callback) {
            return callback(prebuildConfig, args);
        }))
        .then(function () {
...
function filterTargets(arg, targets) {
    var splitedArg = arg.split(path.sep);

    return targets.filter(function (target) {
        var splitedTarget = target.split(path.sep);

        return (splitedArg.length === splitedTarget.length && target === arg) ||
